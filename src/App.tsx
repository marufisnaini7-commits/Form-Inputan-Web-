...
export default function App() {
  ...
  // Filters State
  const [activeTab, setActiveTab] = useState<'input' | 'dashboard' | 'database' | 'integration'>('input');
  ...
  const handleAddRecord = async (newRecordData: Omit<QuarantineRecord, 'id' | 'komoditasSummary' | 'totalVolume' | 'totalNilaiEkonomi'> & {
    ...
    if (storageConfig.mode === 'APPS_SCRIPT_WEBAPP' && storageConfig.appsScriptUrl) {
      ...
      try {
        ...
        setSyncStatus({
          type: 'success',
          message: 'Laporan sukses terkirim dan disimpan di Google Sheets Anda!'
        });
      } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col justify-between">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-6 py-3.5 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0 shadow-sm transition-all duration-300">
        <div id="header-brand" className="flex items-center gap-3.5 text-left w-full md:w-auto">
          <div className="w-10 h-10 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-lg flex items-center justify-center text-white shadow-md shadow-indigo-150 shrink-0">
            <Layers className="w-5.5 h-5.5" />
          </div>
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-base font-extrabold tracking-tight text-slate-900 uppercase">
                Laporan Operasional Perkarantinaan (Implementasi TIFA DAKU)
              </h1>
              <button
                type="button"
            ...
            <p className="text-[10px] font-bold text-slate-500 tracking-wider mt-0.5 uppercase">
              BKHIT PAPUA BARAT DAYA &bull; BADAN KARANTINA INDONESIA
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto">
          <div className="flex items-center bg-indigo-50 px-3.5 py-2 rounded-xl border border-indigo-100 text-xs shadow-xs">
      ...
      <main className="flex-1 w-full max-w-[1550px] mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-md ${
                  activeTab === 'input' ? 'bg-indigo-750 text-indigo-100' : 'bg-slate-100 text-slate-500'
                }`}>
                  Baru
                </span>
              </button>
              ...
              >
                ...
                <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-md ${
                  activeTab === 'dashboard' ? 'bg-indigo-750 text-indigo-100' : 'bg-slate-100 text-slate-500'
                }`}>
                  Grafik
                </span>
              </button>
              ...
              >
                ...
                <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-md ${
                  activeTab === 'database' ? 'bg-indigo-750 text-indigo-100' : 'bg-slate-100 text-slate-500'
                }`}>
              ...
              >
                ...
                <span className={`text-[9px] uppercase font-extrabold px-1.5 py-0.5 rounded-md ${
                  activeTab === 'integration' ? 'bg-indigo-750 text-indigo-100' : 'bg-slate-150 text-slate-500'
                }`}>
                  Sync
                </span>
              </button>
        ...
        <section className="lg:col-span-9 space-y-6">
          ...
          {activeTab === 'integration' && (
            <div className="min-h-[400px] flex items-center justify-center p-2">
              {!isIntegrationUnlocked ? (
                <div className="w-full max-w-md bg-white border border-slate-205 rounded-2xl shadow-xl p-8 text-center space-y-6 animate-fade-in">
                  ...
                  >
                    <div className="relative text-left">
                      ...
                      <input
                        ...
                        className={`w-full text-xs pl-10 pr-4 py-3 rounded-xl border bg-slate-50/55 text-slate-800 focus:outline-none transition-all duration-200 font-semibold ${
                          passwordError 
                    ...
                    <button
                      type="submit"
                      className="w-full bg-indigo-605 hover:bg-indigo-700 bg-indigo-600 text-white font-extrabold py-3 px-4 rounded-xl text-xs shadow-md shadow-indigo-100 transition-all duration-200 cursor-pointer hover:scale-[1.01] active:scale-[0.99] uppercase tracking-wider"
                    >
